...
@Component({
  ...
  directives: [AppChartJs, CHART_DIRECTIVES]
})
export class DashboardPage implements OnInit {
   ...
   @ViewChild('hc') private hc:ElementRef;   
  ...
  ngOnInit() {
    ...
    this.options = {
        ...
        //this.hc.nativeElement.reflow();
        //this.chart.reflow();
    //let e = document.getElementById('hc');
  ...
  chartByRatingBandData(isExpectedLoss: boolean){
        ...
        data.data = this.ratingBandList.map((dataItem) => { 
            for (var item of dataItem.ratingBands) {
              if (item.ratingBand === ratingBands[i]) {
                 if (isExpectedLoss)
                    return Math.round(item.expectedLoss / 1000000);
                 else
                    return Math.round(item.exposure / 1000000);
              }
            }
            return 0;
        });
    ...
    let chartData = {
        ...
        title: {
            text: isExpectedLoss? 'Expected Loss' : 'Exposure' + ' By Rating Band'
        },
        xAxis: {
            categories: xLabels //['Apples', 'Oranges', 'Pears', 'Grapes', 'Bananas']
        },
        yAxis: {
            min: 0,
            title: {
                text: isExpectedLoss? 'Expected Loss' : 'Exposure'
            },
            stackLabels: {
                enabled: false,
                style: {
    ...
    if (isExpectedLoss)
        $('#el-chart').highcharts(chartData);
    else
        $('#exp-chart').highcharts(chartData);
