...
export class CounterpartySearchPage {
  ...
  ionViewDidLoad(){
    ...
    ionViewDidEnter()
    {
        ...
        if(this._isOrientationChanged) this.searchCounterparties(null);
    }

    ionViewDidLeave()
    {
         window.removeEventListener("orientationchange", ()=> this.searchCounterparties(null), false); 
    }
    ...
    Observable.forkJoin(
      ...
      this.instaService.getCounterpartyForwardLimitsAndExposures(counterparty.id)
        ...
        if (data[0] == undefined) {
          ...
          var forwardExposures = _.filter(data[2], function(item){
              return item.id === counterparty.id;
            });  

          this.navCtrl.push(CounterpartyInfoPage, { "counterpartyInfo": data[0], "limitsAndExposures": {"current": data[1], "forward": forwardExposures}});
        }
      },
      error=>{
        loading.dismissAll();
